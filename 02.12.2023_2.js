...
const sum = input.split("\n").reduce((acc, curent) => {
  ...
  const redCounts = Math.max(
    ...redGroup.map((item) => Number(item.split(" ")[0]))
  );
  const greenCounts = Math.max(
    ...greenGroup.map((item) => Number(item.split(" ")[0]))
  );
  const blueCounts = Math.max(
    ...blueGroup.map((item) => Number(item.split(" ")[0]))
  );
